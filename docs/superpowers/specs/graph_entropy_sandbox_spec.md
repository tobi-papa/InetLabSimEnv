...
- All entropies are returned **in bits** (log₂). This is the project-wide convention.
- Use `entropy::util::safe_log(p)` which returns `0` when `p == 0` (with `p > 0` precondition documented), implementing the `0·log(0) = 0` convention by convention at the caller. Provide also `xlogx(p)` returning `p * log(p)` with `p ≤ 0 → 0`. The bit-based forms used by entropy code are `xlog2x(p)` (returning `p * log2(p)` with `p ≤ 0 → 0`) and `safe_log2(p)` (returning `log2(p)` with `p ≤ 0 → 0`).
